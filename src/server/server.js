  ...
  for (let prop in data) {
    if (data.hasOwnProperty(prop)) {
      if (parseInt(data[prop].key) === championId) {
        return {
          name: data[prop].name,
          iconURL: `http://ddragon.leagueoflegends.com/cdn/${latestVersion}/img/champion/${data[prop].name === 'Wukong' ? 'MonkeyKing' : data[prop].name === 'LeBlanc' ? 'Leblanc' : data[prop].name === 'Dr. Mundo' ? 'DrMundo' : data[prop].name}.png`
        }
      }
    }
  }
